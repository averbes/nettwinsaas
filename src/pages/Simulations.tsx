    ...
    <div className="px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t.simulationsTitle}</h1>
            <p className="mt-2 text-gray-600">
              {t.simulationsDescription}
            </p>
          </div>
          <button
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative bg-white rounded-lg max-w-md w-full p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">{t.newSimulation}</h3>
              ...
              <form onSubmit={handleRunSimulation} className="space-y-4">
                <div>
                  ...
                  >
                    <option value="add_link">{t.addLink}</option>
                    <option value="remove_link">{t.removeLink}</option>
                    <option value="change_capacity">{t.changeCapacity}</option>
                  </select>
                </div>
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  >
                    ...
                    {runSimulationMutation.isPending ? 
                      (t.language === 'es' ? 'Ejecutando...' : 'Running...') : 
                      t.runSimulationBtn
                    }
                  </button>
                </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      {simulation.risk_level && (
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getRiskColor(simulation.risk_level)}`}>
                          {t.language === 'es' ? 
                            `riesgo ${t[simulation.risk_level as keyof typeof t] || simulation.risk_level}` :
                    ...
                    {simulation.status === 'completed' && (
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          <span className="text-gray-500">{t.packetLoss}:</span>
                          <span className="ml-2 font-medium">{simulation.packet_loss}%</span>
                        </div>
                        <div>
                          <span className="text-gray-500">{t.latencyImpact}:</span>
